...
if DEBUG == True: print(r'shutdown /m \\'+ ips[comp][0] + ' /'+ mode +r' /t '+str(time) + ' /c "' + comment+ '"')
if mode == 3:	
	if comp == -1:
		for i in range(2, len(ips)+2):
			os.system(r'WakeOnLanC.exe -w -mac ' + ips[i][1])
	else: os.system(r'WakeOnLanC.exe -w -mac ' + ips[comp][1])
else: 
	if comp == -1:
		for i in range(2, len(ips)+2):
	...
	else: os.system(r'shutdown /m \\'+ ips[comp][0] + ' /'+ mode +r' /t '+str(time) + ' /c "' + comment+ '"')
